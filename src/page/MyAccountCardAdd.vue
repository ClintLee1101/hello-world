...
<template>
    <div>
        ...
        <ul class="list60 spaceHT">
            ...
            <li>
                <span>真实姓名</span>
                <input type="text" v-model="clientName" placeholder="请输入真实姓名">
            </li>
            <li>
                <span>身份证</span>
            ...
            <li>
                <span>确认交易密码</span>
                <input v-model="passwordForSure" type="password" placeholder="输入6位数字交易密码">
            </li>
        </ul>
        <div style="margin-top:15px;padding-bottom: 15px;font-size:14px">
            <div style="line-height: 25px;height:25px;width:100px;display:block;float:left">
              <label style="vertical-align: middle;" class="check" :class="alreadyRead?'icon-on':''" @click="checkRead()"></label>
                阅读并同意
            </div>
            <div style="width:66%;float:right;">
                <div class="form-right">
                    <span style="height:25px;line-height: 25px;"><router-link :to="'/protocalService'" style="display:inline-block">《开磷金融使用协议》</router-link></span>
                </div>
                <div class="form-right">
                    <span style="height:25px;line-height: 25px;"><router-link :to="'/register'" style="display:inline-block">《电子交易服务协议》</router-link></span>
                </div>
                <div class="form-right">
                    <span style="height:25px;line-height: 25px;"><router-link :to="'/protocalNotice'" style="display:inline-block">《投资人权益须知》</router-link></span>
                </div>
                <div class="form-right">
                    <span style="height:25px;line-height: 25px;"><router-link :to="'/zlPayService'" style="display:inline-block">《证联支付用户网络支付业务协议》</router-link></span>
                </div>
            </div>
...
<script>
    ...
    export default{
        ...
        methods : {
            ...
            checkRead(){
                this.alreadyRead==true?this.alreadyRead=false:this.alreadyRead=true;
            },
            //关闭弹框
            close(){
                this.options='';
            ...
            submit(){
                if(this.bankCode==''){
                    ...
                    this.showNotice=true;
                    return;
                }else if(this.password.length!=6||isNaN(this.password)){
                    this.options='请输入正确6位数字密码';
                    this.showNotice=true;
                    return;
                }
                else if(this.password!=this.passwordForSure){
                    this.options='两次密码不一致';
                    this.showNotice=true;
                    return;
                }
